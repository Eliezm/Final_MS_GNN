...
class MergeEnv(gym.Env):
    ...
    def _step_real(self, action: int) -> Tuple[Dict, float, bool, bool, Dict]:
        ...
        try:
            # ====================================================================
            # PHASE 0: SYNC FD INDICES FROM LATEST MAPPING ✅ NEW
            # ====================================================================

            logger.info("\n[PHASE 0] SYNCHRONIZING FD INDICES")

            # Get the latest mapping file
            latest_mapping = None
            for i in range(self.current_merge_step, -1, -1):
                ...
                if os.path.exists(mapping_path):
                    ...
                    logger.info(f"  Found latest mapping: iteration {i}")
                    break
