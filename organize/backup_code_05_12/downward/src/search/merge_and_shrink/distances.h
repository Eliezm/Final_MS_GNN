...
public:
    ...
    */
    ...
    /// Return the vector of g‑distances (from init) computed last.
    const std::vector<int> &get_init_distances() const { return init_distances; }

    /// Return the vector of h‑distances (to goal) computed last.
    const std::vector<int> &get_goal_distances() const { return goal_distances; }
