...
class MergeEnv(gym.Env):
    ...
    # FILE: merge_env.py
    # COMPLETE REPLACEMENT for reset() method

    def reset(self, *, seed=None, options=None) -> Tuple[Dict, Dict]:
        """
        ✅ COMPLETE FIX: Reset with FD Index Synchronization

        NOW INCLUDES:
        1. Fresh file detection with timestamps
        2. FD index mapping loading
        3. Proper synchronization before step()
        """
        import time
        import glob
        ...
        logger.info(f"\n{'=' * 80}")
        logger.info(f"RESET INITIATED - {timestamp}")
        logger.info(f"Problem: {os.path.basename(self.problem_file)}")
        logger.info(f"{'=' * 80}\n")

        # ========================================================================
        # PHASE 0: AGGRESSIVE CLEANUP - ENSURE NO STALE DATA
        # ========================================================================
        logger.info("[PHASE 0] AGGRESSIVE CLEANUP\n")

        if self.process and self.process.poll() is None:
            logger.info("  Killing old FD process...")
            try:
                self.process.kill()
                self.process.wait(timeout=2.0)
            except:
                if os.name != 'nt':
                    os.system(f"kill -9 {self.process.pid} 2>/dev/null")
            logger.info("  ✓ FD process killed")
        self.process = None
        ...
        logger.info("  Deleting all signal files...")
        deleted_count = 0
        ...
        fd_files = [f for f in os.listdir(self.fd_output_dir)
                    if f.endswith('.json') and 'debug' not in f]
        if fd_files:
            logger.warning(f"  ⚠️ Warning: {len(fd_files)} JSON files still remain")
        else:
            logger.info(f"  ✓ Directories verified clean\n")
        ...
        # ========================================================================
        # PHASE 1: LAUNCH FD PROCESS
        # ========================================================================
        logger.info("[PHASE 1] LAUNCH FD PROCESS\n")
        ...
        logger.info(f"  Running translator for:")
        logger.info(f"    Domain:  {self.domain_file}")
        logger.info(f"    Problem: {self.problem_file}")
        ...
        try:
            ...
            logger.info("  ✓ Translator completed\n")
        except subprocess.TimeoutExpired:
            raise RuntimeError("Translator timeout")

        # ========================================================================
        # PHASE 2: HANDSHAKE WITH FRESH FILE DETECTION
        # ========================================================================
        logger.info("[PHASE 2] HANDSHAKE WITH FRESH FILE DETECTION\n")

        fd_launch_time = time.time()
        logger.info(f"  FD launch time: {fd_launch_time}\n")
        ...
        logger.info("  Launching FD process...")
        try:
            ...
            logger.info(f"  ✓ FD process running (PID: {self.process.pid})")

            with open(os.path.join(self.fd_base_dir, "output.sas"), "r") as f:
                sas_content = f.read()

            self.process.stdin.write(sas_content)
            self.process.stdin.close()
            logger.info("  ✓ SAS written to stdin\n")

        except Exception as e:
            raise RuntimeError(f"FD launch failed: {e}")

        # ========================================================================
        # PHASE 3: WAIT FOR FRESH FILES (ENHANCED)
        # ========================================================================
        logger.info("[PHASE 3] WAIT FOR FRESH FILES\n")

        cg_path = os.path.join(self.fd_output_dir, "causal_graph.json")

        logger.info("  Deleting any stale causal_graph.json...")
        for _ in range(3):
        ...
        logger.info("  Waiting for FRESH causal_graph.json...")
        logger.info(f"  Cleanup time: {cleanup_time}")
        logger.info(f"  FD launch time: {fd_launch_time}")
        logger.info(f"  Expected files must have mtime >= {fd_launch_time}\n")

        start_wait = time.time()
        deadline = start_wait + 120.0
        file_is_fresh = False
        ...
        while time.time() < deadline:
            ...
            if os.path.exists(cg_path):
                ...
                is_fresh = file_mtime >= fd_launch_time - 1.0

                logger.info(f"  [{elapsed:.1f}s] File exists")
                logger.info(f"    File mtime: {file_mtime}")
                logger.info(f"    FD launch: {fd_launch_time}")
                logger.info(f"    Fresh: {is_fresh}")

                if is_fresh:
                    logger.info(f"\n  ✓ Fresh causal_graph.json detected!\n")
                    file_is_fresh = True
                    break
                else:
                    logger.info("  Stale file detected, re-deleting...")
                    try:
                        os.remove(cg_path)
                    except:
                        pass
        ...
        if not file_is_fresh:
            elapsed = time.time() - start_wait
            raise TimeoutError(f"Fresh causal_graph.json not created after {elapsed:.1f}s")

        # ========================================================================
        # PHASE 4: INITIALIZE GRAPHTRACKER
        # ========================================================================
        logger.info("[PHASE 4] INITIALIZE GRAPHTRACKER\n")
        ...
        try:
            if os.path.exists(ts_path):
                ...
                is_fresh = ts_mtime >= fd_launch_time - 1.0
                logger.info(f"  TS file fresh: {is_fresh}")
                if not is_fresh:
                    logger.warning("  ⚠️ TS file might be stale!")
            ...
            logger.info(f"  ✓ GraphTracker initialized")
            logger.info(f"    Nodes: {self.graph_tracker.graph.number_of_nodes()}")
            logger.info(f"    Edges: {self.graph_tracker.graph.number_of_edges()}\n")

        except Exception as e:
            logger.error(f"  ✗ GraphTracker init failed: {e}")
            raise

        # ========================================================================
        # PHASE 4B: WAIT FOR & SYNC FD INDEX MAPPING ← CRITICAL NEW PHASE
        # ========================================================================
        logger.info("[PHASE 4B] SYNC FD INDEX MAPPING\n")
        ...
        logger.info(f"  Waiting for initial FD index mapping...")
        logger.info(f"  Expected: {initial_mapping_path}\n")

        # Wait for initial mapping file
        start_mapping = time.time()
        mapping_deadline = start_mapping + 60.0
        mapping_found = False

        while time.time() < mapping_deadline:
            if os.path.exists(initial_mapping_path):
                logger.info(f"  ✓ Initial mapping file found!")
                mapping_found = True
                break
            time.sleep(0.2)

        if not mapping_found:
            elapsed = time.time() - start_mapping
            logger.error(f"  ❌ Timeout waiting for initial mapping after {elapsed:.1f}s")
            logger.error(f"  Expected: {initial_mapping_path}")
            raise TimeoutError(f"Initial FD index mapping not created after {elapsed:.1f}s")

        # Sync FD indices from mapping
        try:
            success = self.graph_tracker.sync_fd_indices_from_mapping(initial_mapping_path)
            if not success:
                logger.error("  ❌ Failed to sync FD indices from mapping!")
                raise RuntimeError("FD index synchronization failed")
            logger.info(f"  ✓ Successfully synced FD indices from mapping\n")
        except Exception as e:
            logger.error(f"  ❌ FD index sync failed: {e}")
            raise

        # ========================================================================
        # PHASE 5: INVALIDATE CACHES & BUILD OBSERVATION
        # ========================================================================
        logger.info("[PHASE 5] INVALIDATE CACHES & BUILD OBSERVATION\n")
        ...
        try:
            ...
            logger.warning(f"  ⚠️ Centrality computation failed: {e}")
            self.centrality = {}
        ...
        logger.info(f"  ✓ Initial observation built")
        logger.info(f"    Num nodes: {obs.get('num_nodes')}")
        logger.info(f"    Num edges: {obs.get('num_edges')}")
        logger.info(f"    Total states: {self.prev_total_states}\n")
