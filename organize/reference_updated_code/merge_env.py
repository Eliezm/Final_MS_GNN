...
from reward_info_extractor import RewardInfoExtractor
...
class MergeEnv(gym.Env):
    ...
    def __init__(
        ...
        # ✅ CRITICAL FIX: Use paths from common_utils
        ...
        os.makedirs(self.fd_output_dir, exist_ok=True)
        os.makedirs(self.gnn_output_dir, exist_ok=True)

        logger.info(f"[MERGE_ENV INIT] Using:")
    ...
    def reset(self, *, seed=None, options=None) -> Tuple[Dict, Dict]:
        ...
        # ✅ CRITICAL: Set FD base dir again in case it changed
        self.fd_base_dir = os.path.abspath("downward")

        # ✅ Clean old files (but preserve log.txt which is appended to)
        for folder in ("gnn_output", "fd_output"):
            d = os.path.join("downward", folder)
            if not os.path.isdir(d):
                continue

            # Delete all files EXCEPT log.txt in fd_output
            files_to_delete = []
            for fname in os.listdir(d):
                ...
                # Skip log.txt in fd_output (we append to it)
                if folder == "fd_output" and fname == "log.txt":
                    continue
        ...
        # ✅ NEW: Sync FD indices from initial mapping - WITH WAIT
        logger.info("\n[RESET] Waiting for FD index mapping from C++...")
        initial_mapping_path = os.path.join(self.fd_output_dir, "fd_index_mapping_-1.json")

        # Wait for mapping file to be created (C++ is still initializing)
        max_wait_seconds = 30
        elapsed = 0
        while not os.path.exists(initial_mapping_path) and elapsed < max_wait_seconds:
            time.sleep(0.5)
            elapsed += 0.5
            if elapsed % 5 == 0:
                logger.debug(f"[RESET] Still waiting for FD mapping ({elapsed:.1f}s elapsed)...")

        if os.path.exists(initial_mapping_path):
            logger.info("[RESET] ✓ FD index mapping file found")
            if not self.graph_tracker.sync_fd_indices_from_mapping(initial_mapping_path):
                logger.error("[RESET] ❌ Failed to sync FD indices from mapping")
                raise RuntimeError("Cannot sync FD indices from mapping file")
            logger.info("[RESET] ✓ FD indices synchronized successfully")
        else:
            logger.error(f"[RESET] ❌ FD index mapping not created within {max_wait_seconds}s")
            logger.error(f"[RESET] Expected file: {initial_mapping_path}")
            raise RuntimeError("FD index mapping file not created by C++ process")
        ...
        # ✅ FIX: Use graph_tracker's cached centrality method
        try:
            self.centrality = self.graph_tracker.get_centrality(force_recompute=True)
        except Exception as e:
            logger.warning(f"Failed to compute centrality: {e}, using empty dict")
            self.centrality = {}
    ...
    def step(self, action: int) -> Tuple[Dict, float, bool, bool, Dict]:
        ...
        try:
            # ✅ FIX: Check FD process FIRST and EXPLICITLY
            if self.process is not None:
                ret = self.process.poll()
                if ret is not None:
                    # ✅ FIX: Exit codes: 0=ok, 1-9=clean exit, <0 or >10=crash
                    if ret < 0 or ret > 10:
                        logger.error(f"❌ FD CRASHED: exit code {ret}")
                        return self.state, 0.0, True, False, {"error": "process_crashed"}
                    elif ret > 0:
                        logger.warning(f"⚠️ FD clean exit with code {ret}")
                        return self.state, 0.0, True, False, {"error": "process_exited"}
            ...
            action = int(action)
            num_valid_edges = len(edges)
            # ✅ FIX: Direct clamping, not modulo
            action_idx = int(np.clip(action, 0, num_valid_edges - 1))
    ...
    def _step_real(self, action: int) -> Tuple[Dict, float, bool, bool, Dict]:
        ...
        logger.info("\n" + "=" * 90)
        logger.info(f"STEP {self.current_merge_step}: REAL MODE WITH SYNC")
        logger.info("=" * 90)
