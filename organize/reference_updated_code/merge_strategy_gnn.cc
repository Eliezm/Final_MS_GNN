...
// ============================================================================
// ✅ SIMPLIFIED: Always read from downward/gnn_output
// ============================================================================

std::filesystem::path get_gnn_input_directory() {
    /*
      The GNN always writes to PROJECT_ROOT/downward/gnn_output.
      We're launched from PROJECT_ROOT/downward, so gnn_output is a sibling.
    */
    std::filesystem::path cwd = std::filesystem::current_path();
    ...
    try {
        ...
        std::cerr << "[GNN::PATH] Warning: Could not create gnn_output: " << e.what() << std::endl;
    }
